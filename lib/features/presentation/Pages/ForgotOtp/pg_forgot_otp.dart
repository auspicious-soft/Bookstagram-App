...
import 'package:bookstagram/features/presentation/Pages/ForgotOtp/Widget/change_pass_sheet.dart';
import 'package:bookstagram/features/presentation/providers/forgot_pass_email.dart';
import 'package:bookstagram/features/presentation/providers/forgot_pass_otp.dart';
...
class PgForgotOtp extends ConsumerStatefulWidget {
  final String type;
  const PgForgotOtp({super.key, required this.type});

  @override
  PgForgotOtpState createState() => PgForgotOtpState();
}

class PgForgotOtpState extends ConsumerState<PgForgotOtp> {
  String otpCode = "";

  @override
  Widget build(BuildContext context) {
    final otpVerifyState = ref.watch(stateForgetEmailProvider);
                  ...
                  commonButton(
                      ...
                      onPressed: () {
                        ...
                        if (isValid && otpCode.length == 6) {
                          ref
                              .read(stateForgotCodeNotifierProvider.notifier)
                              .forgotOtp(
                                  otpCode: otpCode,
                                  onSuccess: (fineData) {
  ...
  void goToNext(ForgotOtpModel fineData, BuildContext context) async {
    motion_toast.MotionToast.success(
      ...
      animationType: motion_toast.AnimationType.fromTop,
      position: motion_toast.MotionToastPosition.top,
      dismissable: true,
    ).show(context);
