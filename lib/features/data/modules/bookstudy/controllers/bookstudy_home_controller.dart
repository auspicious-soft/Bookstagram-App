...
class PgBookstudyController extends GetxController {
  ...
  Future<void> fetchBookStudy() async {
    ...
    try {
      ...
      bookStudy.value?.data?.categories = [];
      bookStudy.value?.data?.teachers = [];
      bookStudy.value?.data?.newBooks = [];
      bookStudy.value?.data?.popularCourses = [];
      bookStudy.refresh();
  ...
  Future<BookStudyModel> getListBooks() async {
    try {
      ...
      String uri =
          '${AppConfig.baseUrl}${AppConfig.getBookStudHome}?language=${Uri.encodeComponent(selectedLanguage == "en" ? "eng" : selectedLanguage == "ru" ? "rus" : "kaz")}';
  ...
  @override
  void dispose() {
    debouncer.dispose(); // Clean up debouncer
    super.dispose();
  }
